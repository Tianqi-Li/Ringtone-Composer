...
#import "FirstViewController03.h"
#import <CoreMedia/CoreMedia.h>

#define SOURCE_MOVIE_NAME @"Phantom Love"
#define SOURCE_MOVIE_TYPE @"mp3"


@interface FirstViewController03 () {
    AVAudioPlayer *avPlayer;
}

@end

@implementation FirstViewController03
...
-(IBAction) cutButtonTapped: (id) sender {
	...
	if ((inSeconds > 0) && (outSeconds > 0)) {
        ...
        SecondViewController03 *secondViewController03 = [self.storyboard instantiateViewControllerWithIdentifier:@"SecondViewController03"];
        
        
		[secondViewController03.composition insertTimeRange:editRange
                                                    ofAsset:sourceAsset
                                                     atTime:secondViewController03.composition.duration
                                                      error:&editError];
		if (!editError) {
			// kludge: force the other VC to load its view, so we can touch its goodies
			
			//secondViewController.timeSlider.maximumValue =
            CMTimeGetSeconds (secondViewController03.composition.duration);
            [self presentViewController:secondViewController03 animated:YES completion:nil];
