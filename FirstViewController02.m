...
#import "FirstViewController02.h"
#import <CoreMedia/CoreMedia.h>

#define SOURCE_MOVIE_NAME @"So Close"
#define SOURCE_MOVIE_TYPE @"mp3"

@interface FirstViewController02 () {
    AVAudioPlayer *avPlayer;
}

@end

@implementation FirstViewController02
...
-(IBAction) cutButtonTapped: (id) sender {
	if ((inSeconds > 0) && (outSeconds > 0)) {
        ...
        SecondViewController02 *secondViewController02 = [self.storyboard instantiateViewControllerWithIdentifier:@"SecondViewController02"];
        
        
		[secondViewController02.composition insertTimeRange:editRange
                                                    ofAsset:sourceAsset
                                                     atTime:secondViewController02.composition.duration
                                                      error:&editError];
		if (!editError) {
			// kludge: force the other VC to load its view, so we can touch its goodies
			
			//secondViewController.timeSlider.maximumValue =
            CMTimeGetSeconds (secondViewController02.composition.duration);
            [self presentViewController:secondViewController02 animated:YES completion:nil];
